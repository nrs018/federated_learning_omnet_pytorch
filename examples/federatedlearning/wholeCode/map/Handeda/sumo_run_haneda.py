...
sumoCmd = ["sumo-gui", "-c", "osm_tmc.sumocfg"]
traci.start(sumoCmd)
...
while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for i in range(0,len(vehicles)):
                ...
                laneposition = traci.vehicle.getLanePosition(vehicles[i])
                ...
                bestlane = traci.vehicle.getBestLanes(vehicles[i])  # Information about the wish to use subsequent edges' lanes
                changelane = traci.vehicle.couldChangeLane(vehicles[i],1)
                lanechangemode = traci.vehicle.getLaneChangeMode(vehicles[i])  #Gets the vehicle's lane change mode as a bitset.
                                                                               #Default Lanechangemodel: 1621 --> Execute all changes unless in conflict with TraCI
                                                                               
                
                
                #Packing of all the data for export to CSV/XLSX
                vehList = [getdatetime(), timestep, vehid, coord, gpscoord, spd, edge, lane, laneindex, laneposition, \
                            displacement, turnAngle, nextTLS,bestlane,changelane ,lanechangemode]
                ...
                NEWSPEED1 = 13.8889 # value in m/s (15 m/s = 5 km/hr)
                NEWSPEED2 = 20.8333 # value in m/s (15 m/s = 75 km/hr)               
                NEWSPEED3 = 27.7778 # value in m/s (15 m/s = 100 km/hr)  

                if timestep == 150:
                    if vehicles[i]=='veh2':

                            traci.vehicle.setSpeedMode('veh2',0)
                            traci.vehicle.setSpeed('veh2',NEWSPEED1)
                            
                            
                            if timestep == 300:
                                if vehicles[i]=='veh2':
                                    traci.vehicle.setSpeedMode('veh2',0)
                                    traci.vehicle.setSpeed('veh2',NEWSPEED2)                                    
                                    
                            if timestep == 450:
                                if vehicles[i]=='veh2':
                                    traci.vehicle.setSpeedMode('veh2',0)
                                    traci.vehicle.setSpeed('veh2',NEWSPEED3)                         
                        
                #------------------------------------------------------------------------------#
                      #### Change to the lane whose index is specified in the command  #####
                #if timestep == 120:
                #    if vehicles[i]=='veh1':                    
                #        traci.vehicle.changeLane('veh1', 2, 20)
                        
                #------------------------------------------------------------------------------#

                        #traci.vehicle.setMaxSpeedLat('veh1', 5.00)   #did not work 
                        #traci.vehicle.changeSublane('veh1', 2)       # did not work

                        #traci.vehicle.setSpeedMode('veh1',0)
                        #traci.vehicle.setSpeed('veh1',18)
                        
                #------------------------------------------------------------------------------#
                                #### Change to relative lane with respect to offset #####
                #if timestep == 120:
                    #if vehicles[i]=='veh1':                                           
                        #traci.vehicle.changeLaneRelative('veh1', 2, 20)
                #------------------------------------------------------------------------------#        
                     
                    
                ### setLaneChangeMode(self, vehID, lcm) #Sets the vehicle's lane change mode as a bitset
                
                # **Lanechangemode descriptions **
...
#Generate Excel file
columnnames = ['dateandtime', 'timestep', 'vehid', 'coord', 'gpscoord', 'spd', 'edge', 'lane', 'laneindex', 'laneposition', 'displacement', 'turnAngle', 'nextTLS', 'bestlane',  'changelane', 'lanechangemode', \
