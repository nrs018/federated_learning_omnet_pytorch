//
// Generated file, do not edit! Created by nedtool 5.5 from inet/networklayer/ipv6/Ipv6Header.msg.
//
...
int Ipv6HeaderDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'f' && strcmp(fieldName, "flowLabel") == 0) return base+4;
    if (fieldName[0] == 'h' && strcmp(fieldName, "hopLimit") == 0) return base+5;
    if (fieldName[0] == 'p' && strcmp(fieldName, "protocolId") == 0) return base+6;
    if (fieldName[0] == 'e' && strcmp(fieldName, "extensionHeader") == 0) return base+7;
    return basedesc ? basedesc->findField(fieldName) : -1;
}
...
bool Ipv6HeaderDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    switch (field) {
        ...
        case FIELD_trafficClass: pp->setTrafficClass(string2long(value)); return true;
