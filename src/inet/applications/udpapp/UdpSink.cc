...
void UdpSink::processPacket(Packet *pk)
{
    ...
    std::string a = "/home/narisu/src/omnet/omnetpp-5.5.1/samples/new_toyota_FL/examples/federatedlearning/test_11p_distance/distance.txt";
    std::string d = a;
    fname = d.c_str();
...
//    veins::VeinsInetMobility *sm = check_and_cast<veins::VeinsInetMobility *>(m);
    ...
    outfile << " received packet from " << getFullPath() << " " << simTime() << " " << sm->getCurrentPosition() << " " <<  UdpSocket::getReceivedPacketInfo(pk) << endl;
    outfile.close();
