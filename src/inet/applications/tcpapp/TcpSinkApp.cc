...
void TcpSinkAppThread::dataArrived(Packet *pk, bool urgent)
{
    ...
    //std::cout << simTime() << " " << sinkAppModule->bytesRcvd << endl;
    std::string a = "/home/narisu/src/omnet/omnetpp-5.5.1/samples/new_toyota_FL/examples/federatedlearning/testThroughput/testTP.txt";
    std::string d = a;
    fname = d.c_str();
    if(bytesRcvd == par("modelSize").intValue())
    {
        ...
        cModule * m = getParentModule()->getParentModule()->getSubmodule("mobility", 0);
...
//        veins::VeinsInetMobility *sm = check_and_cast<veins::VeinsInetMobility *>(m);
        ...
        outfile << " tcp closed " << simTime() << " " << getFullPath() << ", posistion:" << sm->getCurrentPosition().x << " , " << sm->getCurrentPosition().y << endl;
        outfile.close();
    }
