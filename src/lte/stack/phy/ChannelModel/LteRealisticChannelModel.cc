...
#include "lte/stack/phy/packet/LteAirFrame.h"
...
#include "lte/stack/phy/layer/LtePhyUe.h"
...
double LteRealisticChannelModel::computeUrbanMacro(double d, bool los)
{
   ...
   if (d > 5000){
       if(tolerateMaxDistViolation_)
           ...
           int xx = 0;
//           throw cRuntimeError("1111Error NLOS urban macrocell path loss model is valid for d <5000 m");
   }
...
double LteRealisticChannelModel::computeRuralMacro(double d, double& dbp, bool los)
{
   ...
   if (d > 5000) {
       if(tolerateMaxDistViolation_)
           ...
           throw cRuntimeError("2222Error NLOS rural macrocell path loss model is valid for d<5000 m");
   }
