...
int D2DModeSwitchNotificationDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'i' && strcmp(fieldName, "interruptHarq") == 0) return base+5;
    if (fieldName[0] == 'c' && strcmp(fieldName, "clearRlcBuffer") == 0) return base+6;
...
bool D2DModeSwitchNotificationDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    D2DModeSwitchNotification *pp = (D2DModeSwitchNotification *)object; (void)pp;
    switch (field) {
        case FIELD_peerId: pp->setPeerId(string2ulong(value)); return true;
        case FIELD_txSide: pp->setTxSide(string2bool(value)); return true;
        case FIELD_oldConnection: pp->setOldConnection(string2bool(value)); return true;
        case FIELD_interruptHarq: pp->setInterruptHarq(string2bool(value)); return true;
        case FIELD_clearRlcBuffer: pp->setClearRlcBuffer(string2bool(value)); return true;
