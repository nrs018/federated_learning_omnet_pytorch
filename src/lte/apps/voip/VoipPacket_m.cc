...
int VoipPacketDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'p' && strcmp(fieldName, "playoutTime") == 0) return base+4;
    if (fieldName[0] == 'p' && strcmp(fieldName, "payloadTimestamp") == 0) return base+5;
...
bool VoipPacketDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    VoipPacket *pp = (VoipPacket *)object; (void)pp;
    switch (field) {
        case FIELD_IDtalk: pp->setIDtalk(string2ulong(value)); return true;
        case FIELD_nframes: pp->setNframes(string2ulong(value)); return true;
        case FIELD_IDframe: pp->setIDframe(string2ulong(value)); return true;
