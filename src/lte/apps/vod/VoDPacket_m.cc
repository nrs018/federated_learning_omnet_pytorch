...
int VoDPacketDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0] == 'p' && strcmp(fieldName, "payloadTimestamp") == 0) return base+4;
...
bool VoDPacketDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    VoDPacket *pp = (VoDPacket *)object; (void)pp;
    switch (field) {
        case FIELD_frameSeqNum: pp->setFrameSeqNum(string2long(value)); return true;
        case FIELD_frameLength: pp->setFrameLength(string2long(value)); return true;
        case FIELD_tid: pp->setTid(string2long(value)); return true;
        case FIELD_qid: pp->setQid(string2long(value)); return true;
